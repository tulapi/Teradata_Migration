...
import snowflake_operations_1 as sf_ops # Import for audit functions
...
# (Helper Functions get_teradata_databases, get_teradata_tables are unchanged)
@st.cache_data(ttl=3600)
def get_teradata_databases():
    st.info("Connecting to Teradata to list databases...")
    try:
        databases = migrator.list_teradata_databases()
...
def connect_to_snowflake():
    try:
        sf_conn = snowflake.connector.connect(
            user=configg.SNOWFLAKE_USER, password=configg.SNOWFLAKE_PASSWORD, account=configg.SNOWFLAKE_ACCOUNT,
            warehouse=configg.SNOWFLAKE_WAREHOUSE, database=configg.SNOWFLAKE_DATABASE, schema=configg.SNOWFLAKE_SCHEMA, role=configg.SNOWFLAKE_ROLE)
        return sf_conn, sf_conn.cursor()
    except Exception as e:
        # We will now handle this error inside the thread
        print(f"Failed to connect to Snowflake: {e}", file=sys.stderr)
        return None, None

# --- MODIFICATION: Wrapper now creates its OWN connection and cursor ---
def run_migration_for_table_wrapper(table_name, database_name, log_stream, migration_details, job_id):
    ...
    if 'migration_status' not in st.session_state: st.session_state.migration_status = {}
    ...
    try:
        # --- MODIFICATION: Connection is created INSIDE the thread ---
        sf_conn, sf_cursor = connect_to_snowflake()
        if not sf_conn:
        ...
        # Execute migration
        migration_result = migrator.migrate_table(table_name, database_name, sf_cursor, log_func=stream_log_func, migration_details=migration_details)
        ...
        # --- MODIFICATION: This block now updates the audit log AND closes the connection ---
        if sf_cursor:
...
# (Database and Table selection UI is unchanged)
with st.sidebar:
    st.subheader("1. Select Teradata Database")
    if st.button("Load Teradata Databases"):
        st.session_state.td_databases = get_teradata_databases()
...
with st.sidebar:
    st.subheader("2. Select Tables for Migration")
    if st.session_state.selected_db:
        if st.button(f"Load Tables from {st.session_state.selected_db}"):
            st.session_state.td_tables, st.session_state.selected_tables = get_teradata_tables(st.session_state.selected_db), []
...
with st.sidebar:
    st.subheader("3. Start Migration")
    ...
    if can_start:
        if st.button("Start Migration Process", type="primary"):
            ...
            st.session_state.executor, st.session_state.futures = concurrent.futures.ThreadPoolExecutor(max_workers=configg.MAX_MIGRATION_WORKERS), []
            st.rerun()
...
if st.session_state.migration_started:
    # --- MODIFICATION: No longer create a global connection here ---
    cols = st.columns(2)
    ...
    if not st.session_state.futures and st.session_state.executor:
        for table in st.session_state.selected_tables:
            ...
            # --- MODIFICATION: We no longer pass the shared cursor ---
            future = st.session_state.executor.submit(run_migration_for_table_wrapper, table, st.session_state.selected_db, log_stream, st.session_state.migration_details, st.session_state.job_id)
    ...
    for table, future in st.session_state.futures:
        ...
        if future.done():
            completed_count += 1
            status_text, color = "Unknown", "gray"
            try:
    ...
    st.info("Use the sidebar to select a Teradata database and tables, then start the migration.")

# (Summary Section is unchanged)
if st.session_state.migration_status and not st.session_state.migration_started:
